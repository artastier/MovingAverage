...
public:
    ...
    Gnuplot& plot_xy(const std::vector<std::pair<numerical,numerical>> &trajectory, const std::string &title = "");
...
//
template<typename numerical>
Gnuplot& Gnuplot::plot_xy(const std::vector<std::pair<numerical,numerical>> &trajectory, const std::string &title)
{
    if (not std::size(trajectory))
    {
        throw GnuplotException("std::vectors too small");
        return *this;
    }
    ...
    // write the data to file
    //
    for (unsigned int i = 0; i < std::size(trajectory); i++)
        tmp << trajectory[i].first << " " << trajectory[i].second << std::endl;
